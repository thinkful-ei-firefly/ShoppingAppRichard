...
check - User can can press a switch/checkbox to toggle between
displying all items or displaying only items that are 
unchecked

check - User can type in a search term and the displayed list will
be filtered by item names only containing that search
term

-create on html
-event listener
-add listener to handle shopping list
-update the STORE
-render with search term
-a way to undo the search- needs some more work!

User can clear a search
-event lisener
-add listener to handle shopping list
-call render(search=false);

check- User can edit the title of an item

-add edit button to item html (render new item html)
-event listener
-add lister to handle shopping list
-get the new name -with prompt?
-update the STORE
-render
...
function handleClearSearchClicked() {
  $('.js-clear-search-form').on('click', event => {
    event.preventDefault();
    renderShoppingList(false);
  });
}

function handleItemEditItemClicked() {
  $('.js-shopping-list').on('click', '.js-item-edit', event => {
    console.log('`handleItemEditItemClicked` ran');
    const id = getItemIdFromElement(event.currentTarget);
    let itemEdit = prompt('');
    console.log(itemEdit);
    
    updateEditItem(itemEdit, id);
    
    renderShoppingList();
  });
}

function updateEditItem(itemEdit, itemId) {
...
function handleSearchForItem() {
  $('#js-search-form').on('submit', function (event) {
    ...
    let searchTerm = $('.js-search-item').val();
    console.log(searchTerm);

    renderShoppingList(searchTerm);
    //update search button to clear search
    $('.js-search-item').val();
    updateSearchButtonHtml();
  });
}

function updateSearchButtonHtml() {
  $('.search').html(`
    <form id="js-clear-form">
      <button type="submit" id='js-clear-button'>Clear Search</button>
    </form>`);
}
...
function renderShoppingList(search = false) {
  ...
  if (search) {
    filteredItems = filteredItems.filter(item => item.name === search);
  }
